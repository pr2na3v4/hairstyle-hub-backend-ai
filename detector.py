...
#                  they are "correct gonial angle" — this is factually wrong.
#                  [172]/[397] are mid-mandible BODY points and systematically
#                  under-read true jaw width by 10-15%.  [132]/[361] stay.
#
...
#                yaw.  Original nose-tip→eye-corner approach was noisy because
#                eye corners shift with gaze and eye-openness.
#
...
#           LINE segment (cross-product formula).
#           AUDIT FIX: previous version computed
#               axis_x = (nose_base_x + chin_x) / 2
#           which is the midpoint x of two points — a DOT, not a line.
#           Comparing nose_tip_x against a midpoint is not a perpendicular
#           distance.  Replaced with the standard cross-product formula.
#
#  VISIBILITY GATING  (Perplexity audit P0 — corrected implementation)
#   All key landmarks tested for visibility ≥ 0.5 before any measurement.
#   Perplexity's suggested fix of  dist * min(vi, vj)  is WRONG — it returns
#   a silently shrunk measurement that corrupts all downstream ratios.
#   Hard rejection is the only correct response to an occluded landmark.
#
# ─────────────────────────────────────────────────────────────────────────────
...
_T = {
    ...
    # ── Square ────────────────────────────────────────────────────────────────
    # All three widths (forehead, cheek, jaw) nearly equal; compact height
    "square_lw_max"    : 1.22,   # compact
    "square_jc_min"    : 0.90,   # strong wide jaw
    "square_fc_min"    : 0.85,   # wide forehead
    "square_eq_max"    : 0.15,   # |fc − jc| uniformity guard
                                  # AUDIT FIX: loosened 0.10 → 0.15
                                  # Camera angle + landmark noise alone produce
                                  # |fc−jc| up to 0.12 on genuine square faces.
                                  # At 0.10 the guard rejected most valid inputs.

    # ── Round ─────────────────────────────────────────────────────────────────
    # Compact + wide forehead + soft jaw
    ...
    # ── Triangle (pear) ───────────────────────────────────────────────────────
    # Jaw wider than forehead; jaw is the absolute widest point
    ...
    # ── Diamond ───────────────────────────────────────────────────────────────
    # Cheekbones dominate; both forehead AND jaw narrower than cheeks
    "diamond_fc_max"   : 0.81,   # narrow forehead vs cheeks
    "diamond_jc_max"   : 0.80,   # narrow jaw vs cheeks
    "diamond_fj_min"   : 0.83,   # lower fj bound — symmetric taper
    "diamond_fj_max"   : 1.10,   # upper fj bound — symmetric taper
                                  # AUDIT FIX: tightened 1.20 → 1.10
                                  # Heart requires fj > 1.22. The old gap of
                                  # 0.02 between Diamond (≤1.20) and Heart
                                  # (>1.22) was too narrow for noisy inputs.
                                  # Now creates a clean 0.12-wide buffer.

    # ── Oval ──────────────────────────────────────────────────────────────────
    # Gentle elongation; balanced proportions — NOT a catch-all
    "oval_lw_min"      : 1.10,   # longer than Square/Round
                                  # AUDIT FIX: lowered 1.22 → 1.10
                                  # Round allows lw ≤ 1.30. A face with
                                  # lw=1.15 and balanced fc/jc couldn't be
                                  # Square, Round, or (old) Oval — fell to
                                  # Indeterminate. Short balanced faces are
                                  # a real face type and belong in Oval.
    ...
    "pose_pitch_max"   : 0.08,   # perpendicular nose deviation / face_width
                                  # AUDIT FIX: raised 0.04 → 0.08
                                  # Natural asymmetry + MediaPipe jitter
                                  # produce 2-5% offsets on straight faces.
                                  # Old 0.04 rejected ~50% of valid inputs.
    "min_face_frac"    : 0.15,   # face_len ≥ 15% of image height
    "vis_threshold"    : 0.50,   # minimum landmark visibility (0-1)

    # ── Confidence  (ideal_value, tolerance) ─────────────────────────────────
    # Gaussian:  conf = exp(−(v − ideal)² / (2 · tolerance²))
    # σ = tolerance  →  exp(−0.5) ≈ 60.7% at exactly ±1 tolerance from ideal
    # Floor: 35% (not 50% — a 50% floor falsely implies half-certainty on bad fits)
    #
    # AUDIT FIX: all (ideal, tolerance) pairs now live in _T — including those
    # previously hardcoded inside _multi_conf call sites (conf_square_fc,
    # conf_diamond_fj). Every tuneable value is now reachable from one place.
...
class FaceMeshDetector:
    ...
    def _vis_ok(self, lm):
        """
        Hard visibility gate — rejects frame if any key landmark is occluded.

        Covers glasses, hair across face, hands, and heavy side-lighting.

        WHY HARD REJECTION and not weighted scaling:
        Perplexity's P0 suggestion of  dist * min(vi, vj)  silently returns a
        shorter distance (e.g. visibility=0.6 → 60% of true jaw width).
        That corrupts every downstream ratio and produces wrong shape labels.
        The only correct response to an occluded landmark is to reject the
        frame and ask the user to retry under better conditions.
        """
        threshold = _T["vis_threshold"]
        return all(lm[i].visibility >= threshold for i in _KEY_LANDMARKS)
    ...
    def _pose_ok(self, lm, w, h):
        ...
        If max/min ratio > pose_yaw_max, head is turned too far and cheek /
        jaw widths will be foreshortened on the near side.

        PITCH (forward / back tilt)
        ─────────────────────────────
        Perpendicular distance from nose-tip [1] to the line defined by
        chin [152] → nose-base [4], normalised by face width.

        Cross-product perpendicular distance formula:
            Line from P1=(x1,y1) to P2=(x2,y2), point P0=(x0,y0):
            d = |(y2−y1)·x0 − (x2−x1)·y0 + x2·y1 − y2·x1|
                ─────────────────────────────────────────────
                          sqrt((y2−y1)² + (x2−x1)²)

        AUDIT FIX: previous code computed
            axis_x = (nose_base_x + chin_x) / 2
        That is the mean x of two points — a dot, not a line.  The
        perpendicular distance from nose-tip to a DOT is simply the 2D
        distance to that dot, which responds differently to pitch vs yaw
        and has no geometric meaning for this purpose.
        """
        # ── yaw ──────────────────────────────────────────────────────────────
    ...
    def _multi_conf(pairs):
        ...
        fit well for the combined score to be high — a single outlier pulls
        the whole score down, which is the desired behaviour for shapes that
        require several simultaneous conditions.

        All (ideal, tolerance) values are sourced from _T so every tuneable
        number lives in one place.  No hardcoded constants here.
        """
        scores = [
    ...
    def classify_shape(self, image: np.ndarray) -> dict:
        ...
        Heart    : forehead >> jaw (fj > 1.22), lw ≤ 1.55
        Triangle : jaw >> forehead (fj < 0.86), jaw widest, lw ≤ 1.55
        Diamond  : cheeks dominate; forehead ≈ jaw (fj in [0.83, 1.10])
        Oval     : gentle elongation lw ∈ [1.10, 1.55], balanced widths
        ...
        # G2: narrow jaw vs cheeks
        # G3: forehead ≈ jaw — symmetric taper on both sides
        #     fj must be in [0.83, 1.10] — asymmetric tapers belong in
        #     Heart (fj > 1.22) or Triangle (fj < 0.86)
